...
def generate_page(slug, title, badge, desc, benefits, who, ms_flag=False):
    # Determine the link to the other language
    ...
    nav_about = "Tentang Saya" if ms_flag else "About Me"
    nav_products = "Produk ▾" if ms_flag else "Products ▾"
    nav_calc = "Kalkulator" if ms_flag else "Calculator"
    nav_partners = "Rakan Kongsi" if ms_flag else "Partners"
    nav_cta = "Hubungi Saya" if ms_flag else "Get Connected"
...
<head>
    ...
    <style>
        ...
        .dropdown-content {{ display: none; position: absolute; top: 100%; left: -150px; background-color: var(--pure-white); min-width: 650px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.1); z-index: 100; border-radius: 12px; padding: 20px; gap: 20px; }}
        ...
        .drop-col a {{ color: var(--text-dark) !important; padding: 8px 0; text-decoration: none; font-size: 0.9rem; transition: color 0.2s; }}
        .drop-col a:hover {{ color: var(--primary-red) !important; padding-left: 5px; }}
        @media (max-width: 992px) {{ .dropdown-content {{ position: static; display: none; min-width: 100%; box-shadow: none; padding: 10px; flex-direction: column; }} .nav-dropdown.active .dropdown-content {{ display: flex; }} }}
    </style>
</head>
<body>
    <!-- Navbar -->
    <nav class="navbar scrolled">
        <div class="container nav-container">
            ...
            <ul class="nav-links">
                ...
                <li class="lang-item">
                    <a href="{other_lang_link}" class="lang-btn"><span>{lang_toggle_text}</span></a>
                </li>
            </ul>
        </div>
    </nav>
...
{faq_html}
            ...
            <div style="text-align: center; padding: 60px; background: var(--pure-white); border-radius: 20px; box-shadow: var(--shadow-lg); border: 1px solid var(--border-color);">
                <h2 style="color: var(--dark-bg); margin-bottom: 20px;">Ready to secure your future?</h2>
                <p style="color: var(--text-muted); margin-bottom: 30px; font-size: 1.1rem;">Get a personalized quotation or speak directly with Anna to find the best configuration for you.</p>
    ...
    <footer class="footer">
        <div class="container footer-container">
            ...
            <div class="footer-links">
                ...
                <ul>
                    <li><a href="../{'index-ms.html' if ms_flag else 'index.html'}#home">{nav_home}</a></li>
                    <li><a href="../{'products-ms.html' if ms_flag else 'products.html'}">{nav_products}</a></li>
                </ul>
            </div>
...
print(f"Generating {len(products_data) * 2} product pages...")
